...
class BaristaRobotOdomToTF(Node):

    def __init__(self):
        super().__init__('odom_to_tf_broadcaster_node')
        robot_namespace = self.get_namespace()
        robot_base_frame=robot_namespace + "/odom" if (robot_namespace != "/") else "/odom"
        self.get_logger().info("namespace is {}, base_frame is {}".format(robot_namespace, robot_base_frame))
        ...
        odom_topic_name =  '{}/odom'.format(robot_namespace) if (robot_namespace != "/") else "/odom"
        ...
        self.get_logger().info("{} odom_to_tf_broadcaster_node ready!".format(robot_namespace))

    def odom_callback(self, msg):
        self.rick_odom = msg
        # print the log info in the terminal
        self.get_logger().debug('Odom VALUE: "%s"' % str(self.rick_odom))
    ...
    def broadcast_new_tf(self):
        ...
        position = self.rick_odom.pose.pose.position
        orientation = self.rick_odom.pose.pose.orientation
...
def main(args=None):
    ...
    rclpy.init()
    odom_to_tf_obj = BaristaRobotOdomToTF()
